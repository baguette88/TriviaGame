$(() => {
  ...
  $($titlescreen).css("background-color", "black");
  ...
  $(".nextQuestion").click(function nextQuestion() {
    ...
    if (ranNums.length > 48) {
      /// REFRESH QUESTIONS IF EXHAUSTED
    ...
    $("<li>")
      .html(obj.results[x].correct_answer)
      .addClass("correct")
      .appendTo($ul);

    $("<li>")
      .html(obj.results[x].incorrect_answers[0])
      .addClass("wrongAnswer")
      .appendTo($ul);

    $("<li>")
      .html(obj.results[x].incorrect_answers[1])
      .addClass("wrongAnswer")
      .appendTo($ul);

    $("<li>")
      .html(obj.results[x].incorrect_answers[2])
      .addClass("wrongAnswer")
      .appendTo($ul);
    ...
    $.fn.shuffleChildren = function () {
      //SHUFFLE LI CHILDREN https://css-tricks.com/snippets/jquery/shuffle-children/
    ...
    $($ul).click(function select() {
      // SPAM BLOCKER! 2 count required to select an answer, each new question ups the counter
    ...
    $("<category>")
      .html(
        obj.results[x].category +
          ". Difficulty Level: " +
          obj.results[x].difficulty +
          "<br>"
      )
      .appendTo($lhead);
    ...
    points++;
    $("<lhead>")
      .html("Question #" + questionNumber + ". " + obj.results[x].question)
      .attr("id", questionNumber)
      .appendTo($lhead); //QUESTION
    ...
    function isCorrect() {
      cl(" checking isCorrect..."); //placed within NEXT question function due to Scoping issues
      if ($(event.target).is(".correct")) {
        ...
        setTimeout(function () {
          nextQuestion();
        }, 1100); // AUTO MVOE TO NEXT QUESTION
        //Play "correct" sound
  ...
  function startTime() {
    function countdown(minutes, seconds) {
      ...
      function tick() {
        ...
        if (ticks > 90) {
          // TURN ORANGE ON 30 SECONDS LEFT
          $("#timer").css("color", "orange");
        }
        //// SWITCH STATEMENT HERE
        if (seconds <= 15 && minutes == 0) {
          // TURN RED ON 15 SECONDS LEFT
          $("#timer").css("color", "red");
        }
  ...
  const openModal = () => {
    ...
    $($modaltextbox).html(
      "Questions Correct: " +
        ...
        highScore +
        "<br>" +
        '<button class="restart" id="restart" href="#"><A HREF="javascript:history.go(0)">Main Menu</button>'
    );
  };
  const closeModal = () => {
    //close Modal
  ...
  function nextQuestion() {
    ...
    $(".canvas").hide();
    $(".canvas").empty(); //slideDown(450)
    $(".canvas").css("border", "2px solid grey"); //STYLING THE QUESTION CANVAS
    $(".canvas").css("background-color", "white"); // CHANGE COLOR AT NEW LEVEL?
    $(".canvas").css("color", "black");
